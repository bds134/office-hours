function doGet(e) {
  try {
    ...
    if (action === 'request') {
      ...
      if (!/@binghamton\.edu$/i.test(email)) {
        return ContentService.createTextOutput('Please use a @binghamton.edu email address')
          .setMimeType(ContentService.MimeType.TEXT);
      }
      ...
      if (formSheet) {
        ...
        formSheet.appendRow([timestamp, email, 'Request outside posted hours', name, email, preference, notes]);
      }

      MailApp.sendEmail({
        to: 'bskopyk@binghamton.edu',
        subject: '📩 Office Hours Request Outside Posted Hours',
        htmlBody: `
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <h2 style="color: #667eea;">Office Hours Request</h2>
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Preferred time window:</strong> ${preference || 'None provided'}</p>
